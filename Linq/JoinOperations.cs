    ...
    public static class JoinOperations
    {
        ...
        public static IEnumerable<(string category, string productName)> JoinQuery()
        {
            ...
            var prodByCategory =
                ...
                join product in products on category equals product.Category into pc
                from p in pc
                select (category, p.ProductName);
        ...
        public static IEnumerable<(string category, string productName)> LeftOuterJoin()
        {
            ...
            var prodByCategory =
                from category in categories
                join product in products on category equals product.Category into pc
                from product in pc.DefaultIfEmpty()
                select (category, ProductName: product == null ? "(No products)" : product.ProductName);
