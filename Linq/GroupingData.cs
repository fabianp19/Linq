...
namespace Linq
{
    ...
    public static class GroupingData
    {
        ...
        public static IEnumerable<IGrouping<char, string>> GroupByProperty()
        {
            ...
            return words.GroupBy(x => x[0]).OrderBy(x => x.Key).Select(x => x);
        }
        ...
        public static IEnumerable<(int remainder, IEnumerable<int> numbers)> Grouping()
        {
            ...
            return numbers.GroupBy(x => x % 5).Select(x => (x.Key, x.Select(y => y)));
        }
        ...
        public static IEnumerable<(string category, IEnumerable<string> productsName)> GroupByCategory()
        {
            ...
            return products.GroupBy(x => x.Category).Where(x => x.Count() <= 7).Select(x => (x.Key, x.Select(y => y.ProductName)));
        }
        ...
        public static IEnumerable<IGrouping<string, string>> GroupByCustomComparer()
        {
            ...
            return anagrams.GroupBy(x => x.Trim(), new AnagramEqualityComparer());
        }
        ...
        public static IEnumerable<IGrouping<string, string>> NestedGroupByCustom()
        {
            ...
            return anagrams.GroupBy(x => x.Trim(), y => y.ToUpper(System.Globalization.CultureInfo.CurrentCulture), new AnagramEqualityComparer());
        }
    }
}
